  ...
  return (
    <section className="panel">
      ...
      <div className="panel" style={{ marginTop: 12, boxShadow: 'none' }}>
        ...
        {quoteItems.map((row) => <div key={row.id} className="table-row"><span>{row.productName || row.productId}</span><span>{row.quantity}</span><span>{formatMoney(row.unitPrice)}</span><span>{formatMoney(row.totalAmount)}</span></div>)}
