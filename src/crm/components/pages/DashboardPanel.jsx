...
function DashboardPanel({
  ...
  const ownerBars = mapToBars(reports?.customerByOwner).map((item) => ({ ...item, label: ownerAlias[item.label] || item.label }))
              ...
              <div className="workbench-col">
                ...
                {todoItems.map((item) => (
                  <button key={`${item.type}-${item.id}`} className="workbench-item" onClick={() => {
                    ...
                    <small>{translateStatus(t, item.status)} · {item.owner || '-'}</small>
                  </button>
                ))}
