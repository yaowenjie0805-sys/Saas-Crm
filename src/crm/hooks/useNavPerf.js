...
export function useNavPerf(logPrefix = '[perf]') {
  ...
  const metricsRef = useRef({
    ...
    cacheTotal: 0,
    workbenchJumpHits: 0,
    workbenchJumpTotal: 0,
    workbenchActionSuccess: 0,
    workbenchActionTotal: 0,
    customer360ActionSuccess: 0,
    customer360ActionTotal: 0,
    customer360JumpHits: 0,
    customer360JumpTotal: 0,
    customer360ModuleCacheHits: 0,
    customer360ModuleCacheTotal: 0,
    customer360PrefetchHits: 0,
    customer360PrefetchTotal: 0,
  ...
  const markCacheDecision = useCallback((hit) => {
    metricsRef.current.cacheTotal += 1
    if (hit) metricsRef.current.cacheHits += 1
    const { cacheHits, cacheTotal } = metricsRef.current
    metricsRef.current.cache_hit_rate = cacheTotal > 0 ? Math.round((cacheHits / cacheTotal) * 100) : 0
  }, [])
  ...
  const markWorkbenchJumpDecision = useCallback((hit) => {
    metricsRef.current.workbenchJumpTotal += 1
    if (hit) metricsRef.current.workbenchJumpHits += 1
    const { workbenchJumpHits, workbenchJumpTotal } = metricsRef.current
    metricsRef.current.workbench_jump_hit_rate = workbenchJumpTotal > 0
      ? Math.round((workbenchJumpHits / workbenchJumpTotal) * 100)
      : 0
  }, [])

  const markWorkbenchActionResult = useCallback((success) => {
    metricsRef.current.workbenchActionTotal += 1
    if (success) metricsRef.current.workbenchActionSuccess += 1
    const { workbenchActionSuccess, workbenchActionTotal } = metricsRef.current
    metricsRef.current.workbench_action_success_rate = workbenchActionTotal > 0
      ? Math.round((workbenchActionSuccess / workbenchActionTotal) * 100)
      : 0
  }, [])
  ...
  const markCustomer360ActionResult = useCallback((success) => {
    metricsRef.current.customer360ActionTotal += 1
    if (success) metricsRef.current.customer360ActionSuccess += 1
    const { customer360ActionSuccess, customer360ActionTotal } = metricsRef.current
    metricsRef.current.customer360_action_success_rate = customer360ActionTotal > 0
      ? Math.round((customer360ActionSuccess / customer360ActionTotal) * 100)
      : 0
  }, [])
  ...
  const markCustomer360JumpHit = useCallback((hit) => {
    metricsRef.current.customer360JumpTotal += 1
    if (hit) metricsRef.current.customer360JumpHits += 1
    const { customer360JumpHits, customer360JumpTotal } = metricsRef.current
    metricsRef.current.customer360_jump_hit_rate = customer360JumpTotal > 0
      ? Math.round((customer360JumpHits / customer360JumpTotal) * 100)
      : 0
    recomputeCustomer360Risk()
  }, [recomputeCustomer360Risk])

  const markCustomer360ModuleCacheHit = useCallback((hit, moduleName = '') => {
    metricsRef.current.customer360ModuleCacheTotal += 1
    if (hit) metricsRef.current.customer360ModuleCacheHits += 1
    const { customer360ModuleCacheHits, customer360ModuleCacheTotal } = metricsRef.current
    metricsRef.current.customer360_module_cache_hit_rate = customer360ModuleCacheTotal > 0
      ? Math.round((customer360ModuleCacheHits / customer360ModuleCacheTotal) * 100)
      : 0
    if (moduleName && metricsRef.current.customer360ModuleCounters[moduleName]) {
  ...
  const markCustomer360PrefetchHit = useCallback((hit, moduleName = '') => {
    metricsRef.current.customer360PrefetchTotal += 1
    if (hit) metricsRef.current.customer360PrefetchHits += 1
    const { customer360PrefetchHits, customer360PrefetchTotal } = metricsRef.current
    metricsRef.current.customer360_prefetch_hit_rate = customer360PrefetchTotal > 0
      ? Math.round((customer360PrefetchHits / customer360PrefetchTotal) * 100)
      : 0
    if (moduleName && metricsRef.current.customer360ModuleCounters[moduleName]) {
