    ...
    <section className="panel" data-testid="leads-page">
      <div className="panel-head">
        ...
        <div className="inline-tools">
          <button className="mini-btn" data-testid="leads-refresh" onClick={refreshSelf}>{t('refresh')}</button>
          <button className="mini-btn" data-testid="leads-download-template" onClick={downloadTemplate}>{t('leadDownloadTemplate')}</button>
        </div>
      </div>
      ...
      <div className="inline-tools filter-row" data-testid="leads-filters">
        ...
        <select data-testid="leads-page-size-filter" className="tool-input" value={pagination.size} onChange={(e) => onSizeChange(Number(e.target.value || 10))}>
          {PAGE_SIZE_VALUES.map((n) => <option key={n} value={n}>{n}</option>)}
        </select>
        <button className="mini-btn" data-testid="leads-search-submit" onClick={applyLeadFilters}>{t('search')}</button>
        <button className="mini-btn" data-testid="leads-search-reset" onClick={resetLeadFilters}>{t('reset')}</button>
      </div>

      {canWrite && (
        <div className="inline-tools filter-row" data-testid="lead-form" style={{ marginTop: 12 }}>
          <input data-testid="lead-form-name" className={`tool-input ${fieldErrors?.name ? 'input-invalid' : ''}`} placeholder={t('leadName')} value={leadForm.name} onChange={(e) => setLeadForm((p) => ({ ...p, name: e.target.value }))} />
          <input data-testid="lead-form-company" className="tool-input" placeholder={t('companyName')} value={leadForm.company} onChange={(e) => setLeadForm((p) => ({ ...p, company: e.target.value }))} />
          <input data-testid="lead-form-phone" className="tool-input" placeholder={t('phone')} value={leadForm.phone} onChange={(e) => setLeadForm((p) => ({ ...p, phone: e.target.value }))} />
          <input data-testid="lead-form-email" className="tool-input" placeholder={t('email')} value={leadForm.email} onChange={(e) => setLeadForm((p) => ({ ...p, email: e.target.value }))} />
          <input data-testid="lead-form-owner" className="tool-input" placeholder={t('owner')} value={leadForm.owner} onChange={(e) => setLeadForm((p) => ({ ...p, owner: e.target.value }))} />
          <input data-testid="lead-form-source" className="tool-input" placeholder={t('leadSource')} value={leadForm.source} onChange={(e) => setLeadForm((p) => ({ ...p, source: e.target.value }))} />
          <select data-testid="lead-form-status" className="tool-input" value={leadForm.status} onChange={(e) => setLeadForm((p) => ({ ...p, status: e.target.value }))}>
          ...
          <button className="mini-btn" data-testid="lead-form-submit" onClick={saveLead} disabled={loading}>{leadForm.id ? t('save') : t('create')}</button>
        </div>
      )}
      ...
      {!!formError && <div className="error-banner" data-testid="lead-form-error" style={{ marginTop: 10 }}>{formError}</div>}
