...
public class V1BatchActionController extends BaseApiController {
    ...
    public ResponseEntity<?> batchContracts(HttpServletRequest request, @RequestBody Map<String, Object> payload) {
        ...
        String ownerValue = str(payload.get("owner"));
        for (String id : ids) {
            Optional<ContractRecord> optional = contractRecordRepository.findByIdAndTenantId(id, tenantId);
            if (!optional.isPresent()) {
                summary.notFound(id, msg(request, "contract_not_found"), traceId(request));
                continue;
            }
            ContractRecord row = optional.get();
            if (isSalesScoped(request) && !ownerMatchesScope(request, row.getOwner())) {
                summary.forbidden(id, msg(request, "scope_forbidden"), traceId(request));
                continue;
            }
    ...
    public ResponseEntity<?> batchPayments(HttpServletRequest request, @RequestBody Map<String, Object> payload) {
        ...
        String ownerValue = str(payload.get("owner"));
        for (String id : ids) {
            Optional<PaymentRecord> optional = paymentRecordRepository.findByIdAndTenantId(id, tenantId);
            if (!optional.isPresent()) {
                summary.notFound(id, msg(request, "payment_not_found"), traceId(request));
                continue;
            }
            PaymentRecord row = optional.get();
            if (isSalesScoped(request) && !ownerMatchesScope(request, row.getOwner())) {
                summary.forbidden(id, msg(request, "scope_forbidden"), traceId(request));
                continue;
            }
