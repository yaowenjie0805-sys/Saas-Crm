...
const DB_URL = process.env.DB_URL || `jdbc:mysql://${HOST}:3306/${DB_NAME}?useUnicode=true&characterEncoding=utf8&serverTimezone=Asia/Shanghai&useSSL=false&allowPublicKeyRetrieval=true`
...
async function prepareArtifacts() {
  ...
  if (IS_WIN) {
    ...
    if (!SKIP_PACKAGE) {
      await run('cmd.exe', ['/c', 'mvn', `-Dmaven.repo.local=${MAVEN_REPO}`, '-f', 'backend/pom.xml', 'package', '-DskipTests'])
    }
    return
  }
  if (!SKIP_FRONTEND_BUILD) {
  ...
  if (!SKIP_PACKAGE) {
    await run('mvn', [`-Dmaven.repo.local=${MAVEN_REPO}`, '-f', 'backend/pom.xml', 'package', '-DskipTests'])
  }
}
...
async function runPlaywright() {
  ...
  if (IS_WIN) {
    await run('cmd.exe', ['/c', 'npx.cmd', 'playwright', 'test'], { env })
    return
  }
...
async function main() {
  ...
  const frontend = startBuffered(
    ...
    IS_WIN ? 'cmd.exe' : 'npm',
    IS_WIN
      ? ['/c', 'npm.cmd', 'run', 'preview', '--', '--host', HOST, '--port', FRONTEND_PORT, '--strictPort']
      : ['run', 'preview', '--', '--host', HOST, '--port', FRONTEND_PORT, '--strictPort'],
    {
      env: {
        ...process.env,
        VITE_API_BASE_URL: API_BASE_URL,
